...
data_map CGenerator::getFunctionBaseTemplateData(Group *group, FunctionBase *fn)
{
    ...
    info["type"] = getTypenameName(dataType, "");
    ...
    info["prototypewt"] = getFunctionPrototypeWithoutReturnValue(group, fn);
...
string CGenerator::getFunctionPrototypeWithoutReturnValue(Group *group, FunctionBase *fn, std::string name)
{
...
{
    DataType *dataTypeReturn = fn->getReturnType();
    string proto = getFunctionPrototypeWithoutReturnValue(group, fn, name);
