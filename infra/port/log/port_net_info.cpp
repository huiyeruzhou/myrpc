...
void sprint_net_info(char *netinfo, int netinfo_len, const sockaddr *__sockaddr,
  ...
  if (__sockaddr->sa_family == AF_INET)
    inet_ntop(AF_INET, &(reinterpret_cast<const sockaddr_in *>(__sockaddr))->sin_addr,
              host, 16);
  else
    snprintf(host, sizeof(host), "AF:%d-notipv4", __sockaddr->sa_family);
  snprintf(netinfo, netinfo_len, "%s:%d", host,
           get_port_from_addr(__sockaddr, __len));
}
...
int get_port_from_addr(const sockaddr *__sockaddr, int __len) {
  if (__sockaddr->sa_family == AF_INET) {
    ...
    auto casted_addr =
        reinterpret_cast<const struct sockaddr_in6 *>(__sockaddr);
    return ntohs(casted_addr->sin6_port);
  } else {
    LOGW("net info log", "unknown SA_FAMILY: %d, interpret as AF_INET",
         __sockaddr->sa_family);
    auto casted_addr = reinterpret_cast<const struct sockaddr_in *>(__sockaddr);
    return ntohs(casted_addr->sin_port);
  }
  return -1;
