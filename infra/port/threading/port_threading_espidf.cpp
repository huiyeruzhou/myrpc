...
// 默认传入后三个参数
erpc::Thread::Thread(thread_entry_t entry, uint32_t priority,
                     uint32_t stackSize, const char *name)
    : m_entry(entry),
      m_arg(0),
...
void erpc::Thread::init(thread_entry_t entry, uint32_t priority,
                        uint32_t stackSize) {
  LOGW(TAG, "init");
  m_entry = entry;
  m_stackSize = stackSize;
...
void erpc::Thread::start(void *arg) {
  ...
  if (pdPASS == xTaskCreate(threadEntryPointStub, (m_name[0] ? m_name : "task"),
...
void erpc::Thread::threadEntryPointStub(void *arg) {
  ...
  assert((_this != NULL) &&
         ("Reinterpreting 'void *arg' to 'Thread *' failed." != NULL));
...
bool erpc::Mutex::lock(void) {
  return (pdTRUE == xSemaphoreTakeRecursive(m_mutex, portMAX_DELAY) ? true
                                                                    : false);
}

bool erpc::Mutex::unlock(void) {
  return (pdTRUE == xSemaphoreGiveRecursive(m_mutex) ? true : false);
}

erpc::Semaphore::Semaphore(int count) : m_sem(0) {
...
int erpc::Semaphore::getCount(void) const {
