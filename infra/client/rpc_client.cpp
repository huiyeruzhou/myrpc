 ...
 *
 * File: rpc_client.cpp
 * Description: the rpc client implementation
 * Version: V1.0.0
 * Date: 2023/08/23
...
erpc::Client::Client(const char *host, uint16_t port) : CSBase(host, port), m_sequence(0) {}
...
rpc_status erpc::Client::performRequest(char *path, const pb_msgdesc_t *req_desc, void *req, const pb_msgdesc_t *rsp_desc, void *rsp)
{
    if (m_transport == NULL) {
    ...
    err = m_transport->to_recv_md->has_status ? static_cast<rpc_status>(m_transport->to_recv_md->status) : UnExpectedMsgType;
    ...
    if (m_sockfd > 0) {
        ...
        if (status == Success) {
            ...
            for (res = res0; res; res = res->ai_next) {
                ...
                timeout.tv_sec = 2;
                timeout.tv_usec = 0;
                set = setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<void *>(&timeout), sizeof(timeout));
                if (set < 0) {
        ...
        if (status == Success) {
            set = 1;
            if (setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<void *>(&set), sizeof(int)) < 0) {
                ::close(sock);
            ...
            timeout.tv_sec = 5;
            timeout.tv_usec = 0;
            set = setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));
            if (set < 0) {
                LOGW(TAG, "setsockopt failed, error: %s", strerror(errno));
                ::close(sock);
                sock = -1;
