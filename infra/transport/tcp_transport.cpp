...
rpc_status erpc::TCPTransport::receive(uint8_t *data, uint32_t size) {
  ...
  if (m_socket <= 0) {
    LOGE(TAG, "m_socket <= 0: %d, data: %s, size: %" PRIu32, m_socket, data,
         size);
...
rpc_status erpc::TCPTransport::recv_inital_md() {
  ...
  if (!md_recvd) {
    CHECK_STATUS(m_codec.get(m_read_buffer, myrpc_Meta_fields, to_recv_md),
                 err);
  }
  return err;
}
...
rpc_status erpc::TCPTransport::recv_trailing_md() {
  return rpc_status::Success;
}
...
rpc_status erpc::TCPTransport::sendFrame() {
  rpc_status ret;
  std::string message;
  message.append(reinterpret_cast<const char *>(m_initial_md_buffer->get()),
                 m_initial_md_buffer->getWritePos());
