...
rpc_status erpc::MessageBuffer::read(void *data, uint32_t length,
                                     uint16_t offset) {
...
rpc_status erpc::MessageBuffer::write(const void *data, uint32_t length,
                                      uint16_t offset) {
  rpc_status err = Success;
...
rpc_status erpc::MessageBuffer::moveReadPos(uint32_t length, uint16_t offset) {
  ...
  if (offset + length > m_len) {
    LOGE(TAG,
         "moveReadPos BufferOverrun: offset = %" PRIu16 ", length = %" PRIu32
         ", m_len = %" PRIu16,
         offset, length, m_len);
...
rpc_status erpc::MessageBuffer::moveWritePos(uint32_t length, uint16_t offset) {
  ...
  if (offset + length > m_len) {
    LOGE(TAG,
         "moveWritePos BufferOverrun: offset = %" PRIu16 ", length = %" PRIu32
         ", m_len = %" PRIu16,
         offset, length, m_len);
    return rpc_status::BufferOverrun;
  }
...
rpc_status erpc::MessageBuffer::prepareForWrite(uint32_t length,
                                                uint16_t offset) {
  ...
  if (dst > m_len) {
    ...
    if (new_buf == NULL) {
      LOGE(TAG,
           "prepareForWrite failed to realloc: offset = %" PRIu16
           ", length = %" PRIu32 ", m_len = %" PRIu16 ", new_len = %" PRIu16,
           offset, length, m_len, new_len);
      return rpc_status::MemoryError;
    }
    LOGW(TAG,
         "prepareForWrite realloc: offset = %" PRIu16 ", length = %" PRIu32
         ", m_len = %" PRIu16 ", new_len = %" PRIu16,
         offset, length, m_len, new_len);
    m_len = new_len;
  }
  return rpc_status::Success;
...
rpc_status erpc::MessageBuffer::prepareForRead(uint32_t length,
                                               uint16_t offset) {
  uint16_t dst = offset + length;
  if (dst > m_len) {
    uint16_t new_len = dst < 2 * m_len ? 2 * m_len : dst;
    ...
    if (new_buf == NULL) {
      LOGE(TAG,
           "prepareForRead failed to realloc: offset = %" PRIu16
           ", length = %" PRIu32 ", m_len = %" PRIu16 ", new_len = %" PRIu16,
           offset, length, m_len, new_len);
      return rpc_status::MemoryError;
    }
    LOGW(TAG,
         "prepareForRead realloc: offset = %" PRIu16 ", length = %" PRIu32
         ", m_len = %" PRIu16 ", new_len = %" PRIu16,
         offset, length, m_len, new_len);
    m_len = new_len;
  }
