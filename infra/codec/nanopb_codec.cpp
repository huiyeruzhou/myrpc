...
rpc_status NanopbCodec::write(MessageBuffer *obuf, const pb_msgdesc_t *fields, const void *src_struct)
{
    ...
    CHECK_STATUS(obuf->write(reinterpret_cast<void *>(&no_compress), sizeof(no_compress)), err);
    uint32_t size_n = htonl((uint32_t)size);
    CHECK_STATUS(obuf->write(reinterpret_cast<void *>(&size_n), sizeof(size_n)), err);
...
rpc_status NanopbCodec::get(MessageBuffer *ibuf, const pb_msgdesc_t *fields, void *dst_struct)
{
    ...
    pb_istream_t istream = pb_istream_from_buffer(ibuf->getRead(), static_cast<size_t>(size));
