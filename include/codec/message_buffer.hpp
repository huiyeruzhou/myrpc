 ...
 * Description: This file defines the message buffer class.
 * Version: V1.0.0
 * Date: 2023/08/23
...
class MessageBuffer {
   public:
    MessageBuffer(void) : m_buf(reinterpret_cast<uint8_t *>(malloc(256 * sizeof(uint8_t)))), m_len(256), m_read_pos(0), m_write_pos(0) {}
    ~MessageBuffer(void) { free(m_buf); }

    MessageBuffer(uint8_t *buffer, uint16_t length) : m_buf(buffer), m_len(length), m_read_pos(0), m_write_pos(0) {}
