...
// Handy macro to test threading model. You can also ERPC_THREADS directly to
// test for threading support, i.e. "#if ERPC_THREADS", because
// ERPC_THREADS_NONE has a value of 0.
...
class Thread {
 public:
  ...
  Thread(thread_entry_t entry, uint32_t priority = 5, uint32_t stackSize = 2048,
         const char *name = "");
   ...
   */
  void init(thread_entry_t entry, uint32_t priority = 5,
...
class Semaphore {
 public:
   ...
   *
   * @param[in] timeoutUsecs Time how long can wait for getting semaphore in
   * [us].
   *
...
#endif  // PORT_THREADING_PORT_THREADING_H_
